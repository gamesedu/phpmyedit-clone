...
/*
*  phpMyEdit  v 1.216 JonMod5(MigratedFrom1.204 - 20110807)
*
*  Changes JonMod5c (20111129)
*		-Fixed the space after comma for ADD record (worked only on change before)
*  Changes JonMod5b (20111128)
*		-Changed = array('oper'  => 'IN', 'value' => '('.join(',', $ids).')'); To solve Checkbox filter Problems
*  Changes jonMod 5:
*      -added space (after comma) to checkbox  (so saved checkbox selections can be wrapped)
*  Changes jonMod 4:
*      -Calendar ,utf8 etc
*
*
...
/* $Platon: phpMyEdit/phpMyEdit.class.php,v 1.204 (JonMod4 ,MigratedTo1216 20110807) 2007-09-16 12:57:07 nepto Exp $ */
...
class phpMyEdit
{
	...
	var $page_type;	// current page type  //JON added for 1216version
	...
	var $buttons;		//JON added for 1216version
	...
	{ return $this->fdd[$k]['select'] == 'M' && ! @$this->fdd[$k]['values']['table']; } //JON added @ during upgrade to 1216version
	function col_has_checkboxes($k)
	{ return $this->fdd[$k]['select'] == 'C' && ! @$this->fdd[$k]['values']['table']; } //JON added @ during upgrade to 1216version
	function col_has_radio_buttons($k)
	{ return $this->fdd[$k]['select'] == 'O' && ! @$this->fdd[$k]['values']['table']; } //JON added @ during upgrade to 1216version
	...
	function listall()        { return $this->inc <= 0; }   //JON added listall()  during upgrade to 1216version
	...
	function hidden($k)       { return stristr(@$this->fdd[$k]['input'],'H'); } //JON added @ during upgrade to 1216version
	function password($k)     { return stristr(@$this->fdd[$k]['input'],'W'); } //JON added @ during upgrade to 1216version
	function readonly($k)     { return stristr(@$this->fdd[$k]['input'],'R') || $this->virtual($k);     } //JON added @ during upgrade to 1216version
	function virtual($k)      { return stristr(@$this->fdd[$k]['input'],'V') && $this->col_has_sql($k); } //JON added @ during upgrade to 1216version
	...
	function sql_connect() /* {{{ */
	{
		...
		@mysql_query("SET NAMES 'utf8'",$this->dbh);  //ADDED By JOHN	
	} /* }}} */
	...
	function sql_insert_id() /* {{{ */
	{
	 ...
	 //JON modified sql_limit during upgrade to 1216version
	...
	function fqn($field, $dont_desc = false, $dont_cols = false) /* {{{ */
	{
		...
		if ($this->add_operation()
			...
			if (isset($this->fdd[$this->fds[$field]]['values']['description']) && ! $dont_desc) {   //JON added isSet() during upgrade to 1216version
			...
			// TODO: remove me ok  //JON removed line during upgrade to 1216version
			} else {
				$ret = $this->sd.'PMEtable0'.$this->ed.'.'.$this->sd.$this->fds[$field].$this->ed;
			}
			// TODO: not neccessary, remove me! ok //JON removed line during upgrade to 1216version
		}
		return $ret;
	} /* }}} */
	
		//JON added get_SQL_main_list_query_parts() (WHOLE NEW function) during upgrade to 1216version
	...
	function get_SQL_query($parts) /* {{{ */
	{
		...
		//JON modified WHOLE switch during upgrade to 1216version
		//switch (strtoupper($parts['type'])) { //orig 1216
		switch (strtolower($parts['type'])) { //modified by jon
			case 'select':
				$ret  = 'SELECT ';
				$ret .= $parts['select'];
				$ret .= ' FROM '.$parts['from'];
	...
	function get_SQL_join_clause() /* {{{ */
	{
		...
		for ($k = 0, $numfds = sizeof($this->fds); $k < $numfds; $k++) {
			...
			if (isset($this->fdd[$main_column]['values']['db'])) {	//JON added isset() during upgrade to 1216version
		...
		if ($this->add_operation() || $this->change_operation() || $this->copy_operation()) {
			...
			for ($k = 0; $k < $this->num_fds; $k++) {
				if ($this->displayed[$k] && ! $this->readonly($k) && ! $this->hidden($k)
						&& (@$this->fdd[$k]['js']['required'] || isset($this->fdd[$k]['js']['regexp']))) { //JON added @ during upgrade to 1216version
					if ($first_required) {
...
function '.$this->js['prefix'].'filter_handler(theForm, theEvent)
{
	...
	function display_add_record() /* {{{ */
	{
		for ($tab = 0, $k = 0; $k < $this->num_fds; $k++) {
			...
			if ($this->col_has_values($k)) {
				...
				echo $this->htmlTextarea($this->cgi['prefix']['data'].$this->fds[$k], $css_class_name, $k, $this->fdd[$k]['default'], $escape); //JON replaced textarea with htmlTextarea during upgrade to 1216version
	...
	function display_copy_change_delete_record() /* {{{ */
	{
		...
		$qparts['type']   = 'select';
		$qparts['select'] = @$this->get_SQL_column_list();  //JON added @ during upgrade to 1216version
		$qparts['from']   = @$this->get_SQL_join_clause();  //JON added @ during upgrade to 1216version
	...
	function display_change_field($row, $k) /* {{{ */ 
	{
		...
		if ($this->col_has_values($k)) {
		    ...
		    echo $this->htmlTextarea($this->cgi['prefix']['data'].$this->fds[$k], $css_class_name, $k, $row["qf$k"], $escape); //JON replaced textarea with htmlTextarea during upgrade to 1216version
		} elseif ($this->col_has_php($k)) {
			echo include($this->fdd[$k]['php']);
		} else {
			$len_props = '';
			...
			echo ($this->readonly($k) ? ' disabled' : '');
			//echo ' name="',$this->cgi['prefix']['data'].$this->fds[$k],'" value="';  //orig
			echo ' name="',$this->cgi['prefix']['data'].$this->fds[$k],'" id="',$this->cgi['prefix']['data'].$this->fds[$k],'" value="'; // BY VICTOR: INPUT ID IS NECESSARY TO JSCALENDAR.
			
			if ($escape) echo htmlspecialchars($row["qf$k"]); //JON modified during upgrade to 1216version
			else echo $row["qf$k"];
			echo '"',$len_props,' />',"\n";
		}
		echo '</td>',"\n";
	...
	function cellDisplay($k, $row, $css) /* {{{ */
	{
		...
		if (intval(@$this->fdd[$k]['trimlen']) > 0 && strlen($value) > $this->fdd[$k]['trimlen']) {  //JON added @ during upgrade to 1216version
			$value = ereg_replace("[\r\n\t ]+",' ',$value);
			...
			/* booleans: */ $multiple = false, $readonly = false, $strip_tags = false, $escape = true, $js = NULL)
	{
		...
		if (! is_array($selected)) {
			...
			foreach($selected as $val) $selected2[]=(string)$val;  //JON modified during upgrade to 1216version
			$selected = $selected2;
		}
		$found = false;
			...
			/* booleans: */ $multiple = false, $readonly = false, $strip_tags = false, $escape = true, $js = NULL)
	{
		...
		if ($multiple) {
			if (! is_array($selected) && $selected !== null) {
				$selected = explode(', ', $selected); //JON This re-joins the checked item
			}
		}
		if (! is_array($selected)) {
	...
	//JON added WHOLE htmlTextArea() function during upgrade to 1216version
	function htmlTextarea($name, $css, $k, $value = null, $escape = true) /* {{{ */
	{
	 ...
	 */	
	function display_list_table_buttons($position) /* {{{ */ //JON modified during upgrade to 1216version
	{
		...
		if($this->display['num_pages']) {
			if ($this->listall()) { //JON modified during upgrade to 1216version
	...
	{
		$ret = '';   //JON added line during upgrade to 1216version
	...
	function display_button($name, $position = 'up') /* {{{ */
	{
		...
		if ($name == 'goto_text') {
			...
			$ret .=' onkeypress="return '.$this->js['prefix'].'filter_handler(this.form, event);" />'; //JON modified during upgrade to 1216version
			return $ret;
		}
		if ($name == 'goto_combo') {
			$disabledgoto = !($this->listall() || ($disablednext && $disabledprev)) ? '' : ' disabled'; //JON modified during upgrade to 1216version
			if ($disabledgoto != '' && $disabled < 0) return; //JON modified during upgrade to 1216version
			$kv_array = array();
		...
		if ($name == 'goto') {
			return $this->htmlSubmit('navop', 'Go to', $this->getCSSclass('goto', $position),
					false, ($this->listall() || ($disablednext && $disabledprev)) ? $disabled : 0);//JON modified during upgrade to 1216version
		}		
	...
	function number_of_recs() /* {{{ */
	{
		$count_parts = array(
				'type'   => 'select',
				'select' => 'count(*)',
				'from'   => @$this->get_SQL_join_clause(),  //JON added @ during upgrade to 1216version
				'where'  => @$this->get_SQL_where_from_query_opts()); //JON added @ during upgrade to 1216version
		$res = $this->myquery($this->get_SQL_main_list_query($count_parts), __LINE__);
		$row = $this->sql_fetch($res, 'n');
		$this->total_recs = $row[0];
	} /* }}} */
     //JON added WHOLE NEW FUNCTION filter_heading() during upgrade to 1216version
	...
	//if ($fields != false) {
     ...
     //JON added WHOLE NEW FUNCTION display_sorting_sequence() during upgrade to 1216version
	function display_sorting_sequence() /* {{{ */
	{
     ...
     //JON added WHOLE NEW FUNCTION display_current_query()  during upgrade to 1216version
	...
	function list_table() /* {{{ */
	{
		...
		$select_recs = $this->key != '' &&
			...
			//JON removed 2 lines during upgrade to 1216version		
		/*
		...
		$this->display_list_table_buttons('up');  //JON removed $listall parameter during upgrade to 1216version		
		if ($this->cgi['persist'] != '') {
		 ...
		 */
		$this->sys_cols  = 0; //JON added  $this-> during upgrade to 1216version		
		$this->sys_cols += intval($this->filter_enabled() || $select_recs); //JON added  $this-> during upgrade to 1216version		
		if ($this->sys_cols > 0) { //JON added  $this-> during upgrade to 1216version		
			$this->sys_cols += intval($this->nav_buttons() //JON added  $this-> during upgrade to 1216version		
		 ...
		 */
		if ($this->sys_cols) {  //JON added  $this-> during upgrade to 1216version		
			echo '<th class="',$this->getCSSclass('header'),'" colspan="',$this->sys_cols,'">'; //JON added  $this-> during upgrade to 1216version		
		...
		echo '</tr>',"\n";
			//JON removed the "Prepare the SQL Query from the data definition file" lines during upgrade to 1216version	
		/*
		 ...
		 * the field names.
		 */
		$qparts = $this->get_SQL_main_list_query_parts(); //JON added during upgrade to 1216version	
		...
		//JON removed MANY LINES of Filter code during upgrade to 1216version	
		// filter
		...
		while ((!$fetched && ($row = $this->sql_fetch($res)) != false)
			...
			if ($this->sys_cols) { /* {{{ */  //JON added $this-> during upgrade to 1216version	
				$key_rec     = $row['qf'.$this->key_num];
				 //JON added some CSS code during upgrade to 1216version
				...
				$deleteTitle = htmlspecialchars($this->labels['Delete']);
					//JON removed some code during upgrade to 1216version
				}
					...
					if (! $this->nav_buttons() || $this->sys_cols > 1) {  //JON added $this-> during upgrade to 1216version	
						echo '</td>',"\n";
					}
					if ($this->nav_buttons()) {
		...
		if (0 && $$var_to_total) {    //JON added 0 && during upgrade to 1216version	
			// do the aggregate query if necessary
			//if ($vars_to_total) {
				$qp = array();
				$qp['type'] = 'select';
				$qp['select'] = $aggr_from_clause;
				$qp['from']   = @$this->get_SQL_join_clause();  //JON added @ during upgrade to 1216version	
				$qp['where']  = @$this->get_SQL_where_from_query_opts(); //JON added @ during upgrade to 1216version
				$tot_query    = @$this->get_SQL_query($qp); //JON added @ during upgrade to 1216version
		...
		$this->display_list_table_buttons('down'); //JON removed $listall during upgrade to 1216version
		$this->form_end();
	} /* }}} */
	...
	function do_add_record() /* {{{ */
	{
		...
		for ($k = 0; $k < $this->num_fds; $k++) {
			if ($this->processed($k)) {
				...
				$newvals[$fd] = is_array($fn) ? join(', ',$fn) : $fn; //JON This adds the , (comma) to the database field  CHANGE mod5b2
			}
		}
	...
	function do_change_record() /* {{{ */
	{
		...
		for ($k = 0; $k < $this->num_fds; $k++) {
			if ($this->processed($k) && !$this->readonly($k)) {
				$fd = $this->fds[$k];
				$fn = $this->get_data_cgi_var($fd);
				$newvals[$this->fds[$k]] = is_array($fn) ? join(', ',$fn) : $fn; //JON This adds the , (comma) to the database field 
				if ($query_oldrec == '') {
	...
	function execute() /* {{{ */
	{
		...
		// Let's do explicit quoting - it's safer //JON added and IF for php 5.3 compatibillity during upgrade to 1216version
		...
		if ($this->saveadd == $this->labels['Save'] || $this->savecopy == $this->labels['Save']) {
			...
			$this->saveadd  = null; // unset($this->saveadd)  //JON modified during upgrade to 1216version
			$this->savecopy = null; // unset($this->savecopy) //JON modified during upgrade to 1216version
			$this->recreate_fdd();
		}
		...
		elseif ($this->savechange == $this->labels['Save']) {
			...
			$this->savechange = null; // unset($this->savechange) //JON modified during upgrade to 1216version
			$this->recreate_fdd();
		}
		...
		elseif ($this->savedelete == $this->labels['Delete']) {
			...
			$this->savedelete = null; // unset($this->savedelete) //JON modified during upgrade to 1216version
			$this->recreate_fdd();
		}
		...
		echo ' - '.base64_decode("Q3JlYXRlZCBieSBNYXJpbmlkaXMgSS4gUEUxOSAtIDIwMTI=");
	...
	function phpMyEdit($opts) /* {{{ */
	{
		...
		//$error_reporting = error_reporting(E_ALL & ~E_NOTICE);  //JON removed during upgrade to 1216version
		...
		$this->buttons = @$opts['buttons'];  //JON added @ during upgrade to 1216version
...
//		$old_page = ceil($this->fm / abs($this->inc)) + 1;
		...
		//error_reporting($error_reporting);  //JON removed during upgrade to 1216version
	} /* }}} */
