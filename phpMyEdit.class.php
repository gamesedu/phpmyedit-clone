...
/* $Platon: phpMyEdit/phpMyEdit.class.php,v 1.220 2015/09/07 16:02:40 igor Exp $ */
...
class phpMyEdit
{
	...
	var $encoding;		// used for htmlspecialchars for now
	var $ext_hsc = null;// external htmlspecialchars: NULL(default PHP) | FALSE(internal) | user_defined_function
	...
	function debug_var($name, $val) /* {{{ */
	{
		if (is_array($val) || is_object($val)) {
			...
			ob_end_clean();
			echo $this->_htmlspecialchars($content);
			echo "</pre>\n";
		} else {
			echo 'debug_var()::<i>',$this->_htmlspecialchars($name),'</i>';
			echo '::<b>',$this->_htmlspecialchars($val),'</b>::',"<br />\n";
		}
	} /* }}} */
	...
	{
		$flags = $this->ssl > 0 ? MYSQL_CLIENT_SSL : null;
	...
	function form_begin() /* {{{ */
	{
		$page_name = $this->_htmlspecialchars($this->page_name);
	...
	if (',$condition,') {';
		...
		alert("';
					if (isset($this->fdd[$k]['js']['hint'])) {
						echo $this->_htmlspecialchars($this->fdd[$k]['js']['hint']);
					} else {
...
function '.$this->js['prefix'].'filter_handler(theForm, theEvent)
{
	...
	function display_add_record() /* {{{ */
	{
		for ($tab = 0, $k = 0; $k < $this->num_fds; $k++) {
			...
			if ($this->col_has_values($k)) {
				...
				echo $this->htmlTextarea($this->cgi['prefix']['data'].$this->fds[$k], $css_class_name, $k, @$this->fdd[$k]['default'], $escape);
				...
				if($escape) echo $this->_htmlspecialchars(@$this->fdd[$k]['default']);
	...
	function display_change_field($row, $k) /* {{{ */ 
	{
		...
		if ($this->col_has_values($k)) {
			...
			echo $this->htmlTextarea($this->cgi['prefix']['data'].$this->fds[$k], $css_class_name, $k, $row["qf$k"], $escape);
		} elseif ($this->col_has_php($k)) {
			echo include($this->fdd[$k]['php']);
		} else {
			$len_props = '';
			...
			if ($escape) echo $this->_htmlspecialchars($row["qf$k"]);
			else echo $row["qf$k"];
			echo '"',$len_props,' />',"\n";
		}
		echo '</td>',"\n";
	...
	function display_password_field($row, $k) /* {{{ */
	{
		...
		echo ($this->readonly($k) ? ' disabled' : '');
		echo ' name="',$this->cgi['prefix']['data'].$this->fds[$k],'" value="';
		echo $this->_htmlspecialchars($row["qf$k"]),'"',$len_props,' />',"\n";
		echo '</td>',"\n";
	} /* }}} */
	...
	function urlDisplay($k, $link_val, $disp_val, $css, $key) /* {{{ */
	{
		...
		$target = isset($this->fdd[$k]['URLtarget'])
			? 'target="'.$this->_htmlspecialchars($this->fdd[$k]['URLtarget']).'" '
			: '';
		$prefix_found  = false;
		$postfix_found = false;
		...
		if (strlen($urllink) <= 0 || strlen($urldisp) <= 0) {
			...
			if ($escape) {
				$urldisp = $this->_htmlspecialchars($urldisp);
			}
			$urllink = $this->_htmlspecialchars($urllink);
	...
	{
		return '<input type="hidden" name="'.$this->_htmlspecialchars($name)
			.'" value="'.$this->_htmlspecialchars($value).'" />'."\n";
	} /* }}} */
	...
	{
		$ret = '<select class="'.$this->_htmlspecialchars($css).'" name="'.$this->_htmlspecialchars($name);
		if ($multiple) {
		...
		foreach ($kv_array as $key => $value) {
			$ret .= '<option value="'.$this->_htmlspecialchars($key).'"';
			...
			$escape     && $value = $this->_htmlspecialchars($value);
			$ret .= '>'.$value.'</option>'."\n";
		}
		$ret .= '</select>';
		return $ret;
			...
			/* booleans: */ $multiple = false, $readonly = false, $strip_tags = false, $escape = true, $js = NULL)
	{
		...
		foreach ($kv_array as $key => $value) {
			...
			$ret .= $this->_htmlspecialchars($name).'[]" value="'.$this->_htmlspecialchars($key).'"';
			if ((! $found || $multiple) && in_array((string) $key, $selected, 1)
					|| (count($selected) == 0 && ! $found && ! $multiple)) {
				$ret  .= ' checked';
				$found = true;
	...
	function htmlTextarea($name, $css, $k, $value = null, $escape = true) /* {{{ */
	{
		...
		$ret .= '>';
		if ($escape) {
			$ret .= $this->_htmlspecialchars($value);
		} else {
			$ret .= $value;
		}
		$ret .= '</textarea>'."\n";
        ...
        if ($method == 'POST') {
            ...
            if (! is_array($origvars)) {
                ...
                foreach ($origvars as $key => $val) {
                    ...
                    $ret == '' || $ret .= '&amp;';
                    $ret .= $this->_htmlspecialchars(rawurlencode($key));
                    $ret .= '=';
                    $ret .= $this->_htmlspecialchars(rawurlencode($val));
                }
            }
	...
	{
		if (strlen($this->ext_hsc) > 0) {
			if (!function_exists($this->ext_hsc)) {	// reset external function to default
				$tmp = $this->ext_hsc;
				$this->ext_hsc = null;
				$this->error('Undefined function', 'Not exist external function "'.$tmp.'"');
			} else { // call user defined function
				return call_user_func_array($this->ext_hsc, array($string));
			}
		}
		if ($this->ext_hsc === false) { // internal
			if (PHP_MAJOR_VERSION == 5
				&& PHP_MINOR_VERSION == 4)
			{
				return htmlspecialchars($string, ENT_COMPAT | ENT_HTML401, $this->deduct_htmlspecialchars_encoding($this->encoding));
			}
			if (PHP_MAJOR_VERSION == 5
				&& PHP_MINOR_VERSION == 6)
			{
				return htmlspecialchars($string, ENT_QUOTES | ENT_HTML5 | ENT_DISALLOWED | ENT_SUBSTITUTE, $this->deduct_htmlspecialchars_encoding($this->encoding));
			}
		}
		// default PHP
	...
	function deduct_htmlspecialchars_encoding($encoding) /* {{{ */
	{
		...
		$this->error('Unknown encoding', 'Encoding "'.$encoding.'" is not in allowed');
		return null;
	} /* }}} */
	...
	function print_vars ($miss = 'Current instance variables')  // debug only /* {{{ */
	{
		...
		echo 'fl=',$this->fl,'   ';
		echo 'fm=',$this->fm,'   ';
		echo 'sfn=',$this->_htmlspecialchars($this->get_sfn_cgi_vars()),'   ';
		echo 'qfn=',$this->qfn,'   ';
		echo 'sw=',$this->sw,'   ';
		echo 'rec=',$this->rec,'   ';
	...
	//if ($fields != false) {
		...
		for ($k = 0; $k < $this->num_fds; $k++) {
			...
			if ($this->password($k)) {
				...
				echo '<input class="',$css_class_name,'" value="',$this->_htmlspecialchars(@$m);
	...
	function display_sorting_sequence() /* {{{ */
	{
		...
		*/
		...
		echo $this->_htmlspecialchars($this->page_name
	...
	function display_current_query() /* {{{ */
	{
		...
		if ($text_query != '') {
			...
			echo '<td class="',$css_class_name,'" colspan="',$this->sys_cols,'">';
			echo '<a class="',$css_class_name,'" href="';
			echo $this->_htmlspecialchars($this->get_server_var('PHP_SELF')
					.'?'.$this->cgi['prefix']['sys'].'fl'.'='.$this->fl
					.'&'.$this->cgi['prefix']['sys'].'fm'.'='.$this->fm
					.'&'.$this->cgi['prefix']['sys'].'qfn'.'='.rawurlencode($this->qfn)
			...
			echo '">',$this->labels['Clear'],'</a></td>',"\n";
			echo '<td class="',$css_class_name,'" colspan="',$this->num_fields_displayed,'">';
			echo $this->labels['Current Query'],': ',$this->_htmlspecialchars($text_query),'</td></tr>',"\n";
		}
	} /* }}} */
	...
	function list_table() /* {{{ */
	{
		...
		if (0) { // DEBUG
			...
			echo '[<b>',$this->_htmlspecialchars($this->qfn),'</b>]';
			echo '[<b>',$this->_htmlspecialchars($this->prev_qfn),'</b>]<br />';
		...
		for ($k = 0; $k < $this->num_fds; $k++) {
			...
			if (! $this->fdd[$fd]['sort'] || $this->password($fd)) {
				...
				echo '<a class="',$css_class_name,'" href="';
				echo $this->_htmlspecialchars($this->page_name.'?'.$this->cgi['prefix']['sys'].'fm'.'=0'
						.'&'.$this->cgi['prefix']['sys'].'fl'.'='.$this->fl
		...
		if ($this->nav_text_links() || $this->nav_graphic_links()) {
			...
			$qpviewStr   = $this->_htmlspecialchars($this->page_name.'?'.join('&',$qpview).$this->qfn);
			$qpcopyStr   = $this->_htmlspecialchars($this->page_name.'?'.join('&',$qpcopy).$this->qfn);
			$qpchangeStr = $this->_htmlspecialchars($this->page_name.'?'.join('&',$qpchange).$this->qfn);
			$qpdeleteStr = $this->_htmlspecialchars($this->page_name.'?'.join('&',$qpdelete).$this->qfn);
		}
		...
		while ((!$fetched && ($row = $this->sql_fetch($res)) != false)
			...
			if ($this->sys_cols) { /* {{{ */
				...
				if ($select_recs) {
					...
					if ($this->nav_text_links() || $this->nav_graphic_links()) {
						$queryAppend = $this->_htmlspecialchars('&'.$this->cgi['prefix']['sys'].'rec'.'='.$key_rec);
						...
						$viewTitle   = $this->_htmlspecialchars($this->labels['View']);
						$changeTitle = $this->_htmlspecialchars($this->labels['Change']);
						$copyTitle   = $this->_htmlspecialchars($this->labels['Copy']);
						$deleteTitle = $this->_htmlspecialchars($this->labels['Delete']);
					}
					...
					if ($this->nav_buttons()) {
						...
						echo '" value="',$this->_htmlspecialchars($key_rec),'"';
	...
	function do_add_record() /* {{{ */
	{
		...
		if ($this->exec_triggers('insert', 'before', isset($oldvals) ? $oldvals : null, $changed, $newvals) == false) {
			return false;
		}
		...
		// After trigger
		if ($this->exec_triggers('insert', 'after', isset($oldvals) ? $oldvals : null, $changed, $newvals) == false) {
			return false;
		}
		return true;
	} /* }}} */
	...
	function do_change_record() /* {{{ */
	{
		...
		// Before trigger
		if ($this->exec_triggers('update', 'before', isset($oldvals) ? $oldvals : null, $changed, $newvals) == false) {
			return false;
		}
		...
		// After trigger
		if ($this->exec_triggers('update', 'after', isset($oldvals) ? $oldvals : null, $changed, $newvals) == false) {
			return false;
		}
		return true;
	} /* }}} */
	...
	function do_delete_record() /* {{{ */
	{
		...
		$newvals = array();
		// Before trigger
		if ($this->exec_triggers('delete', 'before', isset($oldvals) ? $oldvals : null, $changed, $newvals) == false) {
			return false;
		}
		// Real query
		...
		// After trigger
		if ($this->exec_triggers('delete', 'after', isset($oldvals) ? $oldvals : null, $changed, $newvals) == false) {
			return false;
		}
		return true;
	} /* }}} */
	...
	{
		echo '<h1>phpMyEdit error: ',$this->_htmlspecialchars($message),'</h1>',"\n";
		if ($additional_info != '') {
			echo '<hr size="1" />',$this->_htmlspecialchars($additional_info);
		}
		return false;
	} /* }}} */
	...
	function phpMyEdit($opts) /* {{{ */
	{
		...
		$this->ext_hsc   = isset($opts['ext_hsc']) ? $opts['ext_hsc'] : null;
