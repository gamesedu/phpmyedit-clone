...
/* $Platon: phpMyEdit/phpMyEditSetup.php,v 1.52 2014-10-06 03:12:33 nepto Exp $ */
...
#:#                 20060906          #:#
#:#  Modified for PHP 7 by Igor Mino, #:#
#:#                 20170523          #:#
#:#####################################:#
function check_constraints($tb,$fd)
{
  global $dbl;
  $fds = mysqli_query($dbl, 'SHOW CREATE TABLE '.$tb);
  $row = mysqli_fetch_array($fds,  MYSQLI_NUM);
  $tableDef = preg_split('/\n/', $row[1]);
  $constraint_arg="";
...
if (!function_exists('mysqli_field_flags')) {
	function mysqli_field_flags($mysqli_result, $field_offset) {
		$flags = array();
		...
		foreach ($constants['mysqli'] as $c => $n) {
			if (preg_match('/MYSQLI_(.*)_FLAG$/', $c, $m))
				if (!array_key_exists($n, $flags))
					$flags[$n] = $m[1];
			$flags_num = mysqli_fetch_field_direct( $mysqli_result, $field_offset );
			if (is_null($flags_num)) continue;
			$flags_num = $flags_num->flags;
			$result = array();
...
function convert_mysqli_type($mysqli_type) {
	$convert = array(
		'int'        => 'int',
		'double'     => 'real',
		'float'      => 'real',
	...
	foreach ($convert as $needle => $ret) {
		if (stristr($mysqli_type, $needle) !== false) {
			return $ret;
		}
	}
	return $mysqli_type;
...
$dbl  = @mysqli_connect('p:'.$hn, $un, $pw, strlen($ssl) ? MYSQL_CLIENT_SSL : null);
...
if ((!$dbl) or empty($submit)) {
	...
	$result = mysqli_query($dbl, 'SHOW DATABASES');
	$dbs = array();
	while (($row = mysqli_fetch_array($result,  MYSQLI_NUM)) != false) $dbs[] = $row[0];
	...
	$result = mysqli_query($dbl, 'SHOW TABLES FROM '.$db);
	$tbs = array();
	while (($row = mysqli_fetch_array($result,  MYSQLI_NUM)) != false) $tbs[] = $row[0];
	if (is_array($tbs)) foreach ($tbs as $tb) {
		$checked = '';
...
//		<td><i>None</i></td><td><i>No id field required</i></td></tr>
	@mysqli_select_db($dbl, $db);
	$tb_desc = @mysqli_query("DESCRIBE $tb");
	$fds     = mysqli_query($dbl, 'SHOW COLUMNS FROM '.$db.'.'.$tb);
	for ($j = 0; ($finfo = mysqli_fetch_array($fds, MYSQLI_ASSOC)) != false; $j++) {
		$checked = stristr($finfo['Key'], 'primary_key') ? ' checked' : '';
		echo '<tr><td><input',$checked,' type="radio" name="id" value="'.htmlspecialchars($finfo['Field']).'"></td>';
		echo '<td>'.htmlspecialchars($finfo['Field']).'</td>';
		echo '<td>'.htmlspecialchars($finfo['Type']).'</td>';
		echo '<td>'.htmlspecialchars($finfo['Key']).'</td>';
	}
	echo '</table><br>
		<input type="submit" name="submit" value="Submit">
		<input type="submit" name="cancel" value="Cancel">
...
// Type of key field (int/real/string/date etc.)");
	...
	if ($id == '') {
		...
		for ($j = 0; ($finfo = mysqli_fetch_array($fds, MYSQLI_ASSOC)) != false; $j++) {
			if ($finfo['Field'] == $id) {
				echo_buffer("\$opts['key_type'] = '".convert_mysqli_type($finfo['Type'])."';");
				break;
			}
		}
	}
...
");
	...
	@mysqli_select_db($dbl, $db);
	$tb_desc = mysqli_query($dbl, "DESCRIBE $db.$tb");
	$ts_cnt = 0;
	while (($finfo = mysqli_fetch_array($tb_desc, MYSQLI_ASSOC)) != false) {
		$fd = $finfo['Field'];
		$fm = $finfo['Type'];
		$fn = strtr($fd, '_-.', '   ');
		...
		$auto_increment = strstr($finfo['Extra'], 'auto_increment') ? 1 : 0;
		if (substr($finfo['Type'],0,3) == 'set') {
		...
		preg_match('/\((.*)\)/', 'varchar(11)', $matches);
		$maxlen = max(1, intval($matches[1]));
		echo_buffer("  'maxlen'   => ".$maxlen.',');
		// blobs -> textarea
		if ($finfo['Type'] == 'blob') {
		...
		if ((substr($finfo['Type'],0,3) == 'set' || substr($finfo['Type'],0,4) == 'enum')
				&& ! (($pos = strpos($finfo['Type'], '(')) === false)) {
			$indent = str_repeat(' ', 18);
			$outstr = substr($finfo['Type'], $pos + 2, -2);
		...
		if ($finfo['Default'] != '' && $finfo['Default'] != 'NULL') {
			echo_buffer("  'default'  => '".$finfo['Default']."',");
